...
export const mergeParentsAction = jobHandler(`*:${mergeParentsBlueprint.operation}`, async (event, tick) => {
  ...
  for (const [studentId, recordsForStudentId] of recordsByStudentId) {
    if (recordsForStudentId.length > 1) {
      ...
      let parentInfoUpdated: boolean = false;
      for (const secondaryRecord of recordsForStudentId.slice(1)) {
        ...
        /**
         * Attempt to update parent information using secondaryParentOne in order of priority:
        ...
        /**
         * Attempt to update parent information using secondaryParentTwo in order of priority:
         ...
         */
        parentInfoUpdated = masterParentTwo.update(secondaryParentTwo) || 
                            masterParentOne.update(secondaryParentTwo) ||
